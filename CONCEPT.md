...
## 1. VISION DU PROJET
Création d'une Entité IA Virtuelle (VTubeuse) autonome, proactive et persistante, capable d'interagir en temps réel avec un environnement complexe (voix, texte, chat Twitch, système d'exploitation). Le système doit garantir une latence sous la seconde (< 1000ms), une résilience totale aux crashs isolés, et s'exécuter sur une configuration matérielle hybride (Local CPU/GPU + API Cloud).
...
* **CPU Local (Ryzen 9 5950X) :** STT (Whisper.cpp), Text I/O, Vector Database, Orchestrateur, TTS (via ONNX).
...
2.  **Semaine 2 :** Intégration de Kokoro (TTS), Whisper.cpp (STT) et du service Text I/O. Mise en place de la tuyauterie audio virtuelle (VB-Cable).
